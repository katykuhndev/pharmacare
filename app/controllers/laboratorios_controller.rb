class LaboratoriosController < ApplicationController
  ...
  def index
    @query = Laboratorio.ransack(params[:q])
    @laboratorios = @query.result.includes(:comuna).page(params[:page])
  end
  ...
  private
    ...
    def laboratorio_params
      params.require(:laboratorio).permit(:nombre, :contacto, :fono, :direccion, :comuna_id)
    end
end
